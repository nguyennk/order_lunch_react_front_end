  ...
  <li className="list-group-item order-dish-item" key={option.id}>
    <div className="row">
      <div className="dish-image col">
        <ImageZoom
          image={{
            src: option.image_source,
            alt: 'preview image',
            className: 'foody_image',
            style: { width: '100px', height: '100px' },
          }}
          zoomImage={{
            src: option.image_source,
            alt: 'zoomed_preview_image',
          }}
        />
      ...
      <div className="dish-action col">
        <FlatButton
          ...
          custom
        >
          <Icon name="ui-1_bold-add" type="mini" color="black" size="x1" />
        </FlatButton>
      </div>
      <div
        className={`dish-option ${!option.variations.length > 1 ? 'hide' : ''}`}
      >
        <SelectField
          ...
          onChange={val => {
            ...
            others.setVariationOption(
              option.id,
              newVal,
              others.variationOptions
            );
          }}
          clearable={false}
        />
...
const ListDishes = ({ options, error, ...others }) => (
  <div className="list-group eh-list-group">
    {generateListGroupAction(error, options, others)}
  </div>
);
...
const mapDispatchToProps = dispatch => ({
  setVariationOption: (dishId, variation, options) =>
    dispatch(setVariationOptions(dishId, variation, options)),
