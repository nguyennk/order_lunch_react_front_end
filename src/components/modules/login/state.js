...
export const requestLogin = creds => {
    return dispatch => {
        ...
        axios.post(loginEndPointSelector(), creds).then(response => {
            localStorage.setItem(token_name, response.data.jwt_token);
            dispatch(setUserLogin(generateLoginObj(false, true, response.data.jwt_token,
                response.data)));
        }).catch(err => {
            dispatch(setUserLogin(generateLoginObj(false, false, undefined,
...
export function requestValidate(userData) {
    ...
    if (!id_token)
        id_token = localStorage.getItem(token_name);
    if (id_token)
        return dispatch => {
            axios.get(loginEndPointSelector(),
                {
